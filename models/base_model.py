    ...
    def validation(self, dataloader, tb_logger, update=True):
        ...
        for index, data in enumerate(pbar):
            ...
            if self.opt.get('visualize', False):
                ...
                logger = get_root_logger()  
                logger.info(f'{name_x}-{name_y}, geo_err: {geo_err.mean():.4f}') # 

            # print mean geo error of current all pairs
            print(f"current mean geo error: {np.concatenate(geo_errors).mean():.4f}")
