...
def parse_options_debug(root_path, data_type, is_train=True):
    ...
    if is_train:
        ...
        # for resolution 8K, 10K, 12K, 15K

        if data_type =='faust_resolution_ulrssm':
            args.opt = 'options_resolution/train/ulrssm.yaml'

        if data_type =='faust_resolution_ours':
            args.opt = 'options_resolution/train/ours.yaml'
        ...
        if data_type=='smal':
            args.opt = 'options/hybrid_self_supervised/train/smal_F.yaml' 
            # args.opt = 'options/zoomout2_hybrid_self_supervised/train/smal_F.yaml'
        ...
        if data_type =='shrec16_cuts':
            args.opt = 'options/hybrid_self_supervised/train/shrec16_cuts.yaml'

        if data_type =='shrec16_holes':
            args.opt = 'options/hybrid_self_supervised/train/shrec16_holes.yaml'    

    else:
        
        # iso-metric shape matching
        if data_type=='faust':  
            args.opt = 'options_one/test/faust.yaml'
...
def parse_options_resolution(root_path, method_type, resolution, is_train=True):
    parser = argparse.ArgumentParser()
    # parser.add_argument('--opt', type=str, required=True, help='Path to option YAML file.')
    args = parser.parse_args()

    if method_type == 'ours':
        path = 'options/hybrid_self_supervised_resolutions'
    
    if method_type == 'ulrssm':
        path = 'options/ulrssm_resolutions'
    
    if method_type == 'hybrid_ulrssm':
        path = 'options/hybrid_ulrssm_resolutions'

    
    if is_train:

        # for resolution 5k, 8K, 10K, 12K, 15K  # 测试运行时间
        if resolution == '5K':
            args.opt = path + '/train/faust_5k.yaml'

        if resolution == '8K':
            args.opt = path + '/train/faust_8k.yaml'

        if resolution == '10K':
            args.opt = path + '/train/faust_10k.yaml'
        
        if resolution == '12K':
            args.opt = path + '/train/faust_12k.yaml'

        if resolution == '15K':
            args.opt = path + '/train/faust_15k.yaml'

    else:

        if resolution == '5K':
            args.opt = path + '/test/faust_5k.yaml'

        if resolution == '8K':
            args.opt = path + '/test/faust_8k.yaml'

        if resolution == '10K':
            args.opt = path + '/test/faust_10k.yaml'
        
        if resolution == '12K':
            args.opt = path + '/test/faust_12k.yaml'

        if resolution == '15K':
            args.opt = path + '/test/faust_15k.yaml'
