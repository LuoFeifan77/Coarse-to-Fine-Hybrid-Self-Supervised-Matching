...
# for 3D [1, N, k], 这种形式
def spectral_mass_computation3D(elas_evecs, mass):

    elas_evecs = elas_evecs.squeeze()
    mass = mass.squeeze()
    ...
    return Mk.unsqueeze(0), sqrtMk.unsqueeze(0), invsqrtMk.unsqueeze(0)
...
def fmap2pointmap3D(C12, evecs_x, evecs_y):
    """
    Convert functional map to point-to-point map

    Args:
        C12: functional map (shape x->shape y). Shape [1, K, K]
        evecs_x: eigenvectors of shape x. Shape [1, V1, K]
        evecs_y: eigenvectors of shape y. Shape [1, V2, K]
    Returns:
        p2p: point-to-point map (shape y -> shape x). [1, V2]
    """
    return nn_query(torch.bmm(evecs_x, C12.transpose(1, 2)), evecs_y)
...
def hybrid_fmap2pointmap(Cxy, evecs_x, evecs_y, elas_Cxy, elas_evecs_x, elas_evecs_y, mass1, mass2):
    ...
    p2p = nn_query(merged_dataA, merged_dataB)  # 是按照这个拼起来？
